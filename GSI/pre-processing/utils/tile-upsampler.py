...
def resample_tile(input_path: Path, output_path: Path, new_size: int) -> None:
    """zoom level 18のタイルを0.2m解像度にアップサンプリング（cubic補間）"""
...
def run_resampling(input_dir: Path, output_dir: Path) -> dict[str, int]:
    ...
    print(f"Upsampling: {total_files}枚 → {new_size}×{new_size}px/tile (target: {TARGET_RESOLUTION_M}m)")

    jobs = [(tf, output_dir / tf.name, new_size) for tf in tile_files]
    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        future_map = {
            executor.submit(resample_tile, input_path, output_path, size): None
        ...
        for future in as_completed(future_map):
            ...
            if processed % LOG_INTERVAL == 0 or processed == total_files:
                print(f"Upsampling progress: {processed}/{total_files}")
